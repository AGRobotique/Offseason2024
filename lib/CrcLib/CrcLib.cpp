...
void CrcLib::UpdateTime() {
  if (_lastUpdateTime != 0) {
    ...
    // update
    _deltaTime =
        (currentTime < _lastUpdateTime) ? 0 : currentTime - _lastUpdateTime;
...
void CrcLib::MoveTank(int8_t leftChannel, int8_t rightChannel,
                      unsigned char leftMotor, unsigned char rightMotor) {
  SetPwmOutput(leftMotor, leftChannel);
  SetPwmOutput(rightMotor, rightChannel);
}

void CrcLib::MoveTank(ANALOG leftChannel, ANALOG rightChannel,
                      unsigned char leftMotor, unsigned char rightMotor) {
...
void CrcLib::MoveTank(int8_t leftChannel, int8_t rightChannel,
                      unsigned char frontLeftMotor, unsigned char backLeftMotor,
...
void CrcLib::MoveArcade(int8_t forwardChannel, int8_t yawChannel,
                        unsigned char leftMotor, unsigned char rightMotor) {
...
void CrcLib::MoveArcade(ANALOG forwardChannel, ANALOG yawChannel,
                        unsigned char leftMotor, unsigned char rightMotor) {
...
void CrcLib::MoveHolonomic(int8_t forwardChannel, int8_t yawChannel,
                           int8_t strafeChannel, unsigned char frontLeftMotor,
                           unsigned char backLeftMotor,
                           unsigned char frontRightMotor,
                           unsigned char backRightMotor) {
  int8_t frontLeft =
      constrain(forwardChannel - yawChannel - strafeChannel, -128,
                127);  // Determines the power of the front left wheel
  int8_t backLeft =
      constrain(forwardChannel - yawChannel + strafeChannel, -128,
                127);  // Determines the power of the front left wheel
  int8_t frontRight =
...
void CrcLib::MoveHolonomic(ANALOG forwardChannel, ANALOG yawChannel,
                           ANALOG strafeChannel, unsigned char frontLeftMotor,
                           unsigned char backLeftMotor,
                           unsigned char frontRightMotor,
                           unsigned char backRightMotor) {
...
void CrcLib::Initialize(bool buzzer) {
  ...
  if (buzzer) PlayTune(CrcUtility::TUNE_METRO, false);
}
...
void CrcLib::SetDigitalPinMode(unsigned char pin, unsigned char mode) {
  if (!IsSafeDigitalPin(pin)) StopEverythingFromError(ERROR_UNSAFE_DIGITAL_PIN);
...
void CrcLib::SetPwmOutput(unsigned char pin, char value) {
  if (!IsSafePwmPin(pin)) StopEverythingFromError(ERROR_UNSAFE_PWM_PIN);
  ...
  if (value !=
    ...
    mapped_value = map(value * servo->reverse, -128, 127, servo->minPulseWidth,
                       servo->maxPulseWidth);
...
void CrcLib::InitializePwmOutput(unsigned char pin, int minPulseWidth,
                                 int maxPulseWidth) {
...
void CrcLib::InitializePwmOutput(unsigned char pin, int minPulseWidth,
                                 int maxPulseWidth, bool reverse) {
  if (!IsSafePwmPin(pin)) StopEverythingFromError(ERROR_UNSAFE_PWM_PIN);
...
unsigned int CrcLib::GetAnalogInput(unsigned char pin) {
  if (!IsSafeAnalogPin(pin)) StopEverythingFromError(ERROR_UNSAFE_ANALOG_PIN);
...
void CrcLib::StopEverythingFromError(const Note notes[],
                                     const ColorDuration colors[]) {
...
void CrcLib::ShowColorPattern(const ColorDuration pattern[], bool repeat) {
  static SimpleColorPattern static_pattern(&CrcUtility::ColorDuration::END,
                                           false);
